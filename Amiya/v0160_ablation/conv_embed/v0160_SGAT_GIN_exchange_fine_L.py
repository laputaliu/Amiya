...
class Amiya(torch.nn.Module):
    def __init__(
        ...
        for tmp_l in range(nlayer_ST_graph):
            if tmp_l==0:
                self.ST_GIN_layers.append(
                    SuperGATConv(
                        in_channels=self.n_rxn,
                        out_channels=self.nfeature_embed,
                ...
                self.ST_GIN_layers.append(
                    SuperGATConv(
                        in_channels=self.nfeature_embed,
                        out_channels=self.nfeature_embed,
                        heads=self.nhead_SGAT,
                        concat=False,
                        dropout=0.2,
        ...
        # readout function in ST garph
        self.st_SGAT_aggrs = torch.nn.ModuleList()
        for tmp_l in range(nlayer_ST_graph):
            self.st_SGAT_aggrs.append(aggr.SoftmaxAggregation(learn=True))
        ...
        for tmp_l in range(nlayer_met_graph):
            self.met_SGAT_layers.append(
                GINConv(
                    nn=MLP(
                        [self.nfeature_met, 64, 128, 64, self.nfeature_met],
                        dropout=MLP_dr,
                        act=MLP_act,
                        norm=MLP_norm
                    ),
    ...
    def forward(self, x, edge_index, rxn_exp,
        ...
        for tmp_l, gin in enumerate(self.ST_GIN_layers):
            st_embed = F.gelu(gin(torch.add(st_embed, x_identity), edge_index, batch=batch)) if tmp_l > 0 else F.gelu(gin(x_rxn, edge_index, batch=batch))
            # print('st_embed shape {}'.format(st_embed.shape))
                        
            # concat each layer's graph readout
            if tmp_l == 0:
                st_concat = F.gelu(self.st_SGAT_aggrs[tmp_l](torch.add(st_embed, x_identity), batch))
                # (128, 512)
            else:
                st_concat = torch.add(
                        st_concat, 
                        F.gelu(self.st_SGAT_aggrs[tmp_l](st_embed, batch))
                )
                # (128, 512)
        ...
        for tmp_l, sgat in enumerate(self.met_SGAT_layers):
            ...
            met_embed = F.gelu(sgat(x_met, met_edge_index)) if tmp_l == 0 else F.gelu(sgat(torch.add(met_embed, x_identity_met), met_edge_index))
...
class LitAmiya(L.LightningModule):
    ...
    def training_step(self, tmp_graph, n_batch,
        ...
        for tmp_gi in range(self.nlayer_ST_graph):
            if tmp_gi == 0:
                loss_ssl = self.amiya.ST_GIN_layers[tmp_gi].get_attention_loss()
            else:
                loss_ssl += self.amiya.ST_GIN_layers[tmp_gi].get_attention_loss()

        # loss = loss_fba_w * loss_fba + (1-loss_fba_w) * (loss_ssl + loss_recon)
        loss = loss_recon + loss_ssl
...
if __name__ == '__main__':
    ...
    record = True
    run_comment = "{}_v0160_SGAT_exchange_fintuning_L".format(datetime.datetime.now().strftime('%y%m%d-%H%M'))
    ...
    for n_fold, (train_ids, val_ids) in enumerate(kfold_iter.split(train_val_dataset)):
        ...
        checkpoint_path='/data/home/liuzhaoyang/ES/project/MetaSpace/large_ST/v0160_ablation/conv_ablation/v0160_SGAT_GIN_exchange_pre/Amiya_0160_SGAT_GIN_exchange_pre-L/version_0/checkpoints/Amiya_0160_SGAT_GIN_exchange_pretrain-L_epoch=49.ckpt',
        met_edge_index=met_edge_index,
        lr=lr_FT,
        wd=wd_FT,
        record_flag=record,
        ...
        checkpoint_callback = ModelCheckpoint(
                filename="Amiya_0160_SGAT_GIN_exchange_fintuning-L_{epoch:02d}",
        ...
        logger = TensorBoardLogger(save_dir='./v0160_SGAT_GIN_exchange_fine/', name="Amiya_0160_SGAT_GIN_exchange_fine-L")
