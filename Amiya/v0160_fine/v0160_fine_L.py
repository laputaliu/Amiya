...
class LitAmiya(L.LightningModule):
    ...
    def training_step(self, tmp_graph, n_batch,
        ...
        raw_met = tmp_graph.maldi[:tmp_graph.batch_size]
        ...
        loss_cosine = CosineEmbeddingLoss(reduction='mean')
        ...
        if len(self.ref_avi_met_index) > 0:
            ...
            loss_recon_spot = loss_cosine(pre_met[:,self.S_avi_met_index], raw_met[:,self.ref_avi_met_index], target_spot)
            loss_recon_met = loss_cosine(pre_met[:,self.S_avi_met_index].t(), raw_met[:,self.ref_avi_met_index].t(), target_met)
        else:
            target_met = torch.ones(pre_met.size(1), device=self.device)
            loss_recon_spot = loss_cosine(pre_met, raw_met, target_spot)
            loss_recon_met = loss_cosine(pre_met.t(), raw_met.t(), target_met)
        ...
        if len(self.ref_avi_met_index) > 0:
            ...
            spot_corr = torch.nanmedian(corr_cal(pre_met[:,self.S_avi_met_index].t().detach(), raw_met[:,self.ref_avi_met_index].t()))
            # metabolite-wise corr
            corr_cal = ConcordanceCorrCoef(num_outputs=len(self.ref_avi_met_index)).to(self.device)
            met_corr = torch.nanmedian(corr_cal(pre_met[:,self.S_avi_met_index].detach(), raw_met[:,self.ref_avi_met_index]))
        else:
            # spot-wise corr
            corr_cal = ConcordanceCorrCoef(num_outputs=pre_met.size(0)).to(self.device)
            spot_corr = torch.nanmedian(corr_cal(pre_met.t().detach(), raw_met.t()))
            # metabolite-wise corr
            corr_cal = ConcordanceCorrCoef(num_outputs=out_embed.size(1)).to(self.device)
            met_corr = torch.nanmedian(corr_cal(pre_met.detach(), raw_met))
...
if __name__ == '__main__':
    ...
    record = True
    run_comment = "{}_v0160_finetuning_L".format(datetime.datetime.now().strftime('%y%m%d-%H%M'))
    ...
    model = LitAmiya.load_from_checkpoint(
    checkpoint_path='/data/home/liuzhaoyang/ES/project/MetaSpace/large_ST/v0160_pre/v0160_pre/Amiya_0160_pre-L/version_3/checkpoints/Amiya_0160_pre-L_epoch=76.ckpt',
    met_edge_index=met_edge_index,
    lr=lr_FT,
    wd=wd_FT,
    record_flag=record,
    ...
    n_rxn=S_tensor.shape[1],
        ref_avi_met_index = ref_avi_met_index,
        S_avi_met_index = S_avi_met_index
)
    # model.amiya.ref_avi_met_index = ref_avi_met_index
    # model.amiya.S_avi_met_index = S_avi_met_index
    
    # trans rxn flux to met flux
    model.amiya.rxn_gene_map = Linear(
            in_features=S_tensor.size(1),
            out_features=S_tensor.size(0)
        )
    # specify weight as S 
    model.amiya.rxn_gene_map.weight = torch.nn.Parameter(S_tensor, requires_grad=False)
    # change to CosineLoss in maldi data fine-tuning
    model.amiya.loss_recon = CosineEmbeddingLoss(reduction='mean')
    ...
    # the last one graph is treated as test dataset
    train_val_sampler = DataLoader(graph_list[:-1], batch_size=int(np.ceil(train_val_frac*(len(graph_list)-1))),
                                        shuffle=True)
    train_dataset, val_dataset = train_val_sampler

    ##### using custom dataloader
    train_loader = model.train_dataloader(
        # Batch.from_data_list(graph_list[0,1,3,4]),
        train_dataset
        # train_val_dataset
    ...
    checkpoint_callback = ModelCheckpoint(
            filename="Amiya_0160_fine-L_{epoch:02d}",
    ...
    logger = TensorBoardLogger(save_dir='./v0160_fine/', name="Amiya_0160_fine_test-L")
